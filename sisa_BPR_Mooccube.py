...
def main(config_args):
    ...
    config = dict()
    data_generator = Data_for_RecEraser_BPR(args.data_path + args.dataset + '/' + args.attack, args.batch_size,
    ...
    if args.save_flag == 1:
        weights_save_path = './Weights/BPR_SISA/bpr_'
        ...
        for epoch in range(args.epoch):
            ...
            if args.verbose > 0:
                perf_str = '[local_model %d] epoch %d [%.4fs + %.4fs]: train_loss=[%.4f=%.4f + %.4f], [valid,test] auc=[%.4f, %.4f]' \
                           % (i, epoch, t2 - t1, t3 - t2, loss, bpr_loss, reg_loss, valid_auc, test_auc)
                print(perf_str)
