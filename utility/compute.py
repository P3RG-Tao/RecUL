...
def compute_neighbor(data_generator, k_hop=0):
    ...
    paddding = np.concatenate([np.arange(matrix_size).reshape(-1, 1), np.arange(matrix_size).reshape(-1, 1),
                               np.ones(matrix_size).reshape(-1, 1)], axis=-1)
    ...
    return neighbor_set[np.where(neighbor_set < data_generator.n_users)], neighbor_set[np.where(
        neighbor_set >= data_generator.n_users)] - data_generator.n_users
