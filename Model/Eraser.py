...
class RecEraser_BPR(nn.Module):
    def __init__(self, data_config, args):
        super(RecEraser_BPR, self).__init__()
    ...
    def emb_lookup(self, users, items, local_id=None):
        ...
        if local_id is None:
            ...
            return users_emb[:, local_id * self.emb_dim:(local_id + 1) * self.emb_dim], items_emb[:,
                                                                                        local_id * self.emb_dim:(
                                                                                                                            local_id + 1) * self.emb_dim]
    ...
    def compute_bpr_loss(self, users, pos_items, neg_items, bias=None):

        pos_score = torch.mul(users, pos_items).sum(dim=-1)
        neg_score = torch.mul(users, neg_items).sum(dim=-1)
        ...
        reg_loss = (users ** 2).sum() + (pos_items ** 2).sum() + (neg_items ** 2).sum()
        reg_loss = self.decay * reg_loss / users.shape[0]
    ...
    def single_model(self, users, pos_items, neg_items, local_num):

        u_e, pos_i_e = self.emb_lookup(users, pos_items, local_id=local_num)
        _, neg_i_e = self.emb_lookup(users, neg_items, local_id=local_num)


        if self.bias_flag:
            bpr_loss, reg_loss = self.compute_bpr_loss(u_e, pos_i_e, neg_i_e, bias=self.local_biases[local_num])
        else:
            bpr_loss, reg_loss = self.compute_bpr_loss(u_e, pos_i_e, neg_i_e)

        loss = bpr_loss + reg_loss
        return loss, bpr_loss, reg_loss, 0

    def attention_based_agg(self, embs, flag):

        if flag == 0:
            embs_w = torch.exp(torch.einsum('abc,ck->abk', F.relu(
                torch.einsum('abc,ck->abk', embs, self.weights['WA']) + self.weights['BA']), self.weights['HA']))
            embs_w = torch.div(embs_w, torch.sum(embs_w, dim=1, keepdim=True))
        else:
            embs_w = torch.exp(torch.einsum('abc,ck->abk', F.relu(
                torch.einsum('abc,ck->abk', embs, self.weights['WB']) + self.weights['BB']), self.weights['HB']))
            embs_w = torch.div(embs_w, torch.sum(embs_w, dim=1, keepdim=True))
    ...
    def compute_agg_model(self, users, pos_items, neg_items):

        with torch.no_grad():

            u_es, pos_i_es = self.emb_lookup(users, pos_items)
            u_es = u_es.view(-1, self.num_local, self.emb_dim)
            pos_i_es = pos_i_es.view(-1, self.num_local, self.emb_dim)

            _, neg_i_es = self.emb_lookup(users, neg_items)
            neg_i_es = neg_i_es.view(-1, self.num_local, self.emb_dim)


        u_es = torch.einsum('abc,bcd->abd', u_es, self.weights['trans_W']) + self.weights['trans_B']
        pos_i_es = torch.einsum('abc,bcd->abd', pos_i_es, self.weights['trans_W']) + self.weights['trans_B']
        neg_i_es = torch.einsum('abc,bcd->abd', neg_i_es, self.weights['trans_W']) + self.weights['trans_B']


        u_e, u_w = self.attention_based_agg(u_es, 0)
        pos_i_e, i_w = self.attention_based_agg(pos_i_es, 1)
        ...
        if self.bias_flag:
            bpr_loss, reg_loss = self.compute_bpr_loss(u_e_drop, pos_i_e, neg_i_e, bias=self.agg_bias)
        else:
            bpr_loss, reg_loss = self.compute_bpr_loss(u_e_drop, pos_i_e, neg_i_e)
        ...
        batch_ratings = None
        loss = bpr_loss + reg_loss
        return loss, bpr_loss, reg_loss, 0, batch_ratings, u_w
