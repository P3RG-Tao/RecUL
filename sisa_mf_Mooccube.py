...
def main(config_args):
    ...
    config = dict()
    data_generator = Data_for_RecEraser_MF(args.data_path + args.dataset + '/' + args.attack, args.batch_size,
                                           args.part_type, args.part_num, args.part_T)
    data_generator.set_train_mode(args.data_type)
    config['n_users'] = data_generator.n_users
    config['n_items'] = data_generator.n_items

    total_time = [0] * args.part_num
    ...
    if args.save_flag == 1:
        weights_save_path = './Weights/MF_SISA/mf_'
    ...
    for i in range(args.part_num):
        ...
        for epoch in range(args.epoch):
            ...
            for batch_data in train_loader_i:
                users, items, labels = batch_data[:, 0].cuda().long(), batch_data[:, 1].cuda().long(), batch_data[:,
                                                                                                       2].cuda().float()
                model.zero_grad()
            ...
            if is_best:
                ...
                user_emb = model.user_embedding.weight[:,
                           i * model.emb_dim: (i + 1) * model.emb_dim].detach().cpu().numpy()
    ...
    for i in range(args.part_num):
        ...
        with open(weights_save_path_local, 'rb') as f:
            u_emb, i_emb = pickle.load(f)
            user_emb_list.append(u_emb)
            item_emb_list.append(i_emb)

    # ===================== 标准 SISA 聚合：所有子模型输出取平均 =====================
    print("\n=== 标准 SISA 聚合推理（子模型输出直接平均）===")
    with torch.no_grad():
        # 获取所有子模型的预测分数
        val_pred_list = []
        test_pred_list = []
        ...
        # SISA：直接平均所有子模型输出
        val_pred_np = np.mean(np.hstack(val_pred_list), axis=1)
        test_pred_np = np.mean(np.hstack(test_pred_list), axis=1)

    # ===================== 计算 AUC、AUCAND、AUCOR（完全对齐你原来的指标）=====================
    valid_auc_sisa = roc_auc_score(valid_data[:, 2], val_pred_np)
    test_auc_sisa = roc_auc_score(test_data[:, 2], test_pred_np)

    # 计算 AND / OR 指标（复用你原有逻辑）
    valid_auc_or, valid_auc_and = 0.0, 0.0
    test_auc_or, test_auc_and = 0.0, 0.0
    ...
    valid_auc_or = roc_auc_score(valid_data[:, 2], val_or)
    test_auc_or = roc_auc_score(test_data[:, 2], test_or)

    # AND：所有子模型都预测为正
    val_and = np.min(val_all_preds, axis=1)
    test_and = np.min(test_all_preds, axis=1)
    valid_auc_and = roc_auc_score(valid_data[:, 2], val_and)
    test_auc_and = roc_auc_score(test_data[:, 2], test_and)

    # ===================== 输出结果 =====================
    print("\n" + "=" * 50)
    print("✅ 标准 SISA 聚合结果（子模型直接平均）")
    print("=" * 50)
    print(f"Valid AUC      = {valid_auc_sisa:.6f}")
    print(f"Valid AUC OR   = {valid_auc_or:.6f}")
    print(f"Valid AUC AND  = {valid_auc_and:.6f}")
    print(f"Test AUC       = {test_auc_sisa:.6f}")
    print(f"Test AUC OR    = {test_auc_or:.6f}")
    print(f"Test AUC AND   = {test_auc_and:.6f}")
    print("=" * 50)

    # 时间统计
    print("\nTraining Time Summary:")
    for i in range(args.part_num):
        print(f"local{i + 1}: {total_time[i]:.6f}s")
    print(f"Total train time: {sum(total_time):.6f}s")
